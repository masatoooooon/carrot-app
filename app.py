...
@st.dialog("📖 独自指標の計算式と意味", width="large")
def indicators_explanation_dialog():
    st.markdown("""
    ### 🛡️ 脚元安心度 (管囲体重比)
    * **計算式:** `(管囲 ÷ 馬体重) × 100` (%)
    * **意味:** 体重に対して脚の骨（管囲）がどれくらい太いかを示します。数値が大きいほど脚元がしっかりしており、怪我のリスクが低いと考えられる指標です。

    ### 🛒 お買い得度 (体重単価)
    * **計算式:** `募集価格 ÷ 馬体重` (万円/kg)
    * **意味:** 1kgあたりの値段を割り出します。数値が小さいほど、価格の割に馬格が立派な「お買い得馬」であると考えられます。

    ### 🫀 心肺機能＆パワー指標 (胸囲体高比)
    * **計算式:** `(胸囲 ÷ 体高) × 100` (%)
    * **意味:** 背の高さに対して、どれだけ胸が深いか（ガッチリしているか）を示します。数値が大きいほど心肺機能が高く、パワー型の馬（ダートや短距離、タフな馬場向き）である傾向があります。

    ### 🏃‍♂️ ムキムキ度／距離適性 (馬体BMI)
    * **計算式:** `馬体重 ÷ 体高` (kg/cm)
    * **意味:** 1cmあたりの体重を示します。数値が高い（重い）と筋肉ムキムキの「短距離（スプリンター）体型」、数値が低い（軽い）とすらっとした「長距離（ステイヤー）体型」という推測の目安になります。
    """)
...
with col_btn:
    if st.button("📖 指標の計算式と意味を見る", use_container_width=True):
        indicators_explanation_dialog()
